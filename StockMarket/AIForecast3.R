...
GetFeature <- function(df) {
    df[, c(-1)] %>% map(rescale) %>% unlist %>% as.vector
...
Train <- function(O, L1N, L2N, L1D, L2D, Epoch, Batch) {
  ...
  validation_split = 0.2
    ...
    tibble(Accuracy, Correct, AccuracyAll, CorrectAll, LatestAccuracy)
   # tibble(model = list(model))
}
testlabel %>% sum
Model <- Train(512, 256, 0.4, 0.3, 20, 40)

Model4 <- Train(0, 120, 240, 0.3, 0.26, 8, 10)
...
std <- apply(Test, 2, sd)
scale(Test, center = mean, scale = std) %>% unlist %>% as.vector

Test2 %>% GetFeature == scale(Test, center = mean, scale = std) %>% unlist %>% as.vector
rescale <- function(x) {
    mean <- mean(x)
    std <- sd(x)
    scale(x, center = mean, scale = std)
}
