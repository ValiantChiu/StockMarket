...
PriceAll_201001_201902 <- rbind(PriceAll_201001_201811_unnest, Price201812_unnest, Price201901_unnest) %>% group_by(公司代號, 公司名稱) %>% nest
PriceAll_201001_201902$data[[1]] %>% arrange(desc(日期))
    ...
    if (Type == 'Bull') {
        GetLabel <- function(BuySellPrice) {
            GetBuyLabel <- function(MA8, MA21, MA55, MA8D, MA21D, MA55D, 最高價, HKY, HKY2, MA8DD, MA21DD, MA55DD) {
                MAVector <- c(MA8, MA21, MA55)
                MAHB <- MAVector %>% max
                MALB <- MAHB * (1 - 0.005)
                MACount <- (MAVector >= MALB) %>% sum
                ifelse(MACount==3 & MA55D>=0,1,0)
            }
...
BullResult <- PriceAll_201001_201902 %>% filter(公司代號 != '3530' & 公司代號 != '8497' & 公司代號 != '6416' & 公司代號 != '6288' & 公司代號 != '3711' & 公司代號 != '1587' & 公司代號 != '3008' & 公司代號 != '911619' & 公司代號 != '911622' & 公司代號 != '1418' & 公司代號 != '1470' &
...
GetMAFeature <- function(Stock, Price) {
    print(Stock)
    Price %>% filter(complete.cases(.))%>%  arrange(日期) %>% AddMVandKD %>% GetMAD %>% GetMADD %>% arrange(日期) %>% filter(complete.cases(.))
}

PriceMA<-PriceAll_201001_201902 %>% mutate(MA = map2(公司代號, data, GetMAFeature))
GetLabel <- function(公司代號, BuySellPrice) {
    print(公司代號)
    GetBuyLabel <- function(MA8, MA21, MA55, MA8D, MA21D, MA55D, 最高價, MA8DD, MA21DD, MA55DD) {
        MAVector <- c(MA8, MA21, MA55)
        MAHB <- MAVector %>% max
        MALB <- MAHB * (1 - 0.005)
        MACount <- (MAVector >= MALB) %>% sum
        ifelse(MACount == 3 & MA55D >= 0 & MA55DD>=0, 1, 0)
    }
    BuySellPrice %>% mutate(BuyLabel = pmap(list(MA8, MA21, MA55, MA8D, MA21D, MA55D, 最高價,  MA8DD, MA21DD, MA55DD), GetBuyLabel) %>% as.character)
}

PriceMAResult <- PriceMA %>% select(-data) %>% mutate(Result = map2(公司代號, MA, GetLabel))
PriceMAResult <- PriceMAResult %>% select(-MA)
PriceMAResultUnest <- PriceMAResult %>% unnest

PriceMAResultUnest %>% filter(BuyLabel == 1) %>% filter(日期 == '107/03/13') %>% .$公司代號


PriceMAResultUnest$日期 %>% unique