...
GetProfitOld <- function(PredictWindow, ExpectedProfitPercent) {
    print(PredictWindow)
    TimeWindow <- 10
    TradeFarePercent <- 0.1425
    GovernmentTaxPercent <- 0.1
...
GetProfit <- function(PredictWindow, ExpectedProfitPercent) {
    ...
    SampleNumber <- (nrow(RFPrice) - PredictWindow)
    Result <- c()
    for (i in 1:SampleNumber) {
        PW <- RFPrice[(i + 1):(i + PredictWindow),]
        Label <- GetLabel(PW, TradeFarePercent, GovernmentTaxPercent, ExpectedProfitPercent)
        Result[i] <- Label
    }
    Times <- (Result %>% sum)
    Profit <- (Result %>% sum) * ExpectedProfitPercent
    tibble(Times, Profit)
}

RFPrice <- Price0051 %>% filter(!is.na(最高價)) %>% CleanPrice %>% GetPriceDifference

OptimizationResult <- tibble(PW = 2:10)
EP<-list(seq(0.1,5,by=0.2))
...
Profit <- OptimizationResultUnnest %>% group_by(PW) %>% filter(Profit==max(Profit))
